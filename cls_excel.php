...
class cls_excel{
    ...
    public function CreateSheetInfo_Export($fileName,$sheetName,$cellNames,$cellWidth,$data,$sheetIndex=0){
        $this->charIndex=range('A','Z');
        $arr=array();   //
        for($i=0;$i<sizeof($cellNames);$i++){
            $arr[$this->charIndex[$i].'1']=$cellNames[$i];
        }
...
/**
    ...
    private function ExportExcel($sheet){
        try {
            ...
            foreach($sheet['cellWidht'] as $k=>$v){
                $objPHPExcel->getActiveSheet()->getColumnDimension($this->charIndex[$k])->setWidth($v);
            }
            foreach ($sheet['cellName'] as $k => $v) {
                $objSheet = $objSheet->setCellValue($k, $v);
            }
            ...
            self::browser_export('Excel5',"{$sheet['fileName']}.xls");//输出到浏览器
